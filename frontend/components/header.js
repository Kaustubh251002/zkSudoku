...
export default function Header() {
  ...
  const renderConnectWallet = () => {
    if (!accountQuery.data?.address) {
      ...
      return (
        <button
          className="text-lg font-medium rounded-md px-5 py-3 bg-red"
          onClick={() => {
            switchNetwork();
          }}
        >
  ...
  return (
    <header className="flex flex-wrap justify-between p-5 mb-10">
      <Link href="/">
        <a className="text-xl md:mb-auto mb-5 font-bold text-transparent bg-clip-text bg-red-400 hover:bg-red-600">
          Zeroku
        </a>
      </Link>
